...
namespace SharpADWS.Methods.ACL
{
    internal class Group
    {
        ...
        public Group(ADWSConnection adwsConnection)
        {
        ...
        public void Run(string Trustees, string Rights, int Rid, string OutputFormat)
        {
            ...
            List<ADObject> userObjects = enumerateRequest.Enumerate("(objectClass=group)", this.DefaultNamingContext, "subtree", new string[] { "distinguishedName", "nTSecurityDescriptor" });

            Console.WriteLine();

            foreach (ADObject userObject in userObjects)
            {
                if (userObject.Class == "group")
                {
